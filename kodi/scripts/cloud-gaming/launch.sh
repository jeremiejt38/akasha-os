...
"$DOCKER" run --rm --name "$CONTAINER_NAME"   --privileged   --network host   -v /tmp:/tmp   -v /dev:/dev   -v /run/udev:/run/udev   -v /storage/.config/cloud-gaming:/data   -v /storage/.kodi/scripts/cloud-gaming/entrypoint.sh:/entrypoint.sh:ro   --entrypoint /entrypoint.sh   -e DISPLAY=:0   -e XDG_RUNTIME_DIR=/tmp   akasha-chromium   chromium     --no-sandbox     --kiosk     --start-fullscreen     --disable-infobars     --disable-session-crashed-bubble     --noerrdialogs     --user-data-dir=/data     --enable-features=VaapiVideoDecoder     --use-gl=egl     --enable-gpu-rasterization     --window-size=1920,1080     "$URL"
